 ...
 *
 * Rider self-service profile editor.
 * Editable: display name, first/last name, phone, vehicle details, photo, password.
 * Read-only (shown but locked): organization, availability, email, account role.
 */
session_start();
...
$stmt = $conn->prepare("
    SELECT r.rider_id, r.image, r.full_name, r.vehicle_type,
           ...
           r.organization, r.is_available, r.created_at AS rider_since,
           a.first_name, a.last_name, a.email, a.phone_number, a.username
    FROM riders r
...
<head>
  ...
  <style>
    ...
    .field-input {
      width:100%; font-size:.875rem; border:1.5px solid #e5e7eb; border-radius:.75rem;
      padding:.625rem .875rem; outline:none; transition:border-color .15s;
      font-family:'Lexend',sans-serif; color:#1f2937; background:#fff;
    }
...
<header class="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between sticky top-0 z-30 shadow-sm">
  <div class="flex items-center gap-3">
    <a href="dashboard.php"
       class="size-8 flex items-center justify-center rounded-xl hover:bg-gray-100 transition-colors text-gray-500">
  ...
  <!-- Availability badge (read-only, shown for info) -->
  ...
  <form method="POST" action="functions/update.php" enctype="multipart/form-data" id="profile-form">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-5 space-y-4">
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <input class="field-input" id="first_name" name="first_name" type="text"
                 value="<?= $v('first_name') ?>" required maxlength="50">
        </div>
        <div>
      ...
      <div>
        ...
        <input class="field-input" id="phone_number" name="phone_number" type="tel"
               value="<?= $v('phone_number') ?>" maxlength="20" placeholder="+63 9XX XXX XXXX">
      </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-5 space-y-4">
      ...
      <div class="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input class="field-input" id="vehicle_plate_number" name="vehicle_plate_number"
                 type="text" value="<?= $v('vehicle_plate_number') ?>"
                 required maxlength="20" placeholder="ABC 1234"
      ...
      <div>
        ...
        <input class="field-input" id="variant_color" name="variant_color"
               type="text" value="<?= $v('variant_color') ?>"
               maxlength="60" placeholder="e.g. Red Honda Click 160">
      </div>

      <div>
        <label class="field-label" for="contact_number">Rider Contact Number <span class="text-red-400">*</span></label>
        <input class="field-input" id="contact_number" name="contact_number"
               type="tel" value="<?= $v('contact_number') ?>"
               required maxlength="20" placeholder="+63 9XX XXX XXXX">
        <p class="text-[11px] text-gray-400 mt-1">Shown to admins and customers on the tracking page.</p>
      </div>

      <!-- Read-only: organization + availability -->
      <hr class="section-divider">
      <div class="grid grid-cols-2 gap-3">
        <div>
          <label class="field-label">Organization</label>
          <input class="field-input readonly" type="text"
                 value="<?= $v('organization') ?: 'Not set' ?>" disabled>
        </div>
        <div>
          <label class="field-label">Availability</label>
          <input class="field-input readonly" type="text"
                 value="<?= $rider['is_available'] ? 'Available' : 'Offline' ?>" disabled>
        </div>
      </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-5 space-y-4">
      ...
      <div>
        ...
        <div class="pw-wrap">
          <input class="field-input" id="new_password" name="new_password"
          ...
          <button type="button" class="pw-eye" onclick="togglePw('new_password','eye1')">
            <svg id="eye1" class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <div>
        ...
        <div class="pw-wrap">
          <input class="field-input" id="confirm_password" name="confirm_password"
                 type="password" autocomplete="new-password"
                 placeholder="Repeat new password">
          <button type="button" class="pw-eye" onclick="togglePw('confirm_password','eye2')">
            <svg id="eye2" class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/>
            </svg>
          </button>
        </div>
    ...
    <div class="pt-1">
      ...
      <p class="text-center text-xs text-gray-400 mt-2">
        Rider since <?= date('F j, Y', strtotime($rider['rider_since'])) ?>
      </p>
    </div>
...
function previewAvatar(input) {
  ...
  reader.onload = e => {
    // Replace whatever is in avatar-wrap with a fresh <img>
...
function removeAvatar() {
  const wrap = document.getElementById('avatar-wrap');
  const initials = (
    (document.getElementById('first_name')?.value?.[0] ?? '?') +
    (document.getElementById('last_name')?.value?.[0]  ?? '?')
  ).toUpperCase();
...
function togglePw(inputId, iconId) {
  ...
  // Swap icon to slashed/unslashed
...
function checkMatch() {
  ...
  const cp = confPwEl.value;
  if (!np && !cp) { matchHint.classList.add('hidden'); return; }
  matchHint.classList.remove('hidden');
  if (np === cp) {
    matchHint.textContent = '✓ Passwords match';
    matchHint.className = 'text-[11px] mt-1 text-green-600';
  } else {
    matchHint.textContent = '✕ Passwords do not match';
    matchHint.className = 'text-[11px] mt-1 text-red-500';
  }
}
...
// Auto-show flash as toast too (for mobile visibility)
<?php if ($flash): ?>
