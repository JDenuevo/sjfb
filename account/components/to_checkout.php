          ...
          <div class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <?php
                // Fetch cities directly — $conn is already available from checkout.php
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Expose so cart_process.js detects we are on checkout
  window.updateDeliveryFee = reload;

  // ── reload: refreshes totals from server ──────────────────────────────────
  function reload() {
  ...
  // Cities are server-rendered — fire reload immediately if one is pre-selected
  if (citySelect && citySelect.value !== '') {
      reload();
  }

  // ── City change → reload totals ───────────────────────────────────────────
  if (citySelect) {
  ...
  (function () {
    ...
    // Text inputs and textarea — debounced
    ...
    // City select and payment radios — save immediately on change
