...
function updateRiderAvailability($conn, $riderId) {
    ...
    $updateStmt = $conn->prepare("UPDATE riders SET is_available = ? WHERE rider_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (isset($_POST['assign_rider'])) {
        ...
        // Set rider as busy when assigning new delivery
        if ($userType === 'rider' || $userType === 'super_admin') {
    ...
    elseif (isset($_POST['mark_delivered'])) {
        ...
        // Smart rider availability update
        if ($userType === 'rider' || $userType === 'super_admin') {
            // Get the rider ID for this order
            ...
            if ($riderResult->num_rows > 0) {
                ...
                if ($riderId) {
                    ...
                    $activeDeliveriesStmt = $conn->prepare("
                        SELECT COUNT(*) as active_count 
                        FROM orders 
                        WHERE assigned_rider_id = ? AND order_status = 'OutForDelivery' AND order_id != ?
                    ");
                    $activeDeliveriesStmt->bind_param("ii", $riderId, $orderId);
                    $activeDeliveriesStmt->execute();
                    $activeResult = $activeDeliveriesStmt->get_result();
                    $activeCount = $activeResult->fetch_assoc()['active_count'];
                    
                    // Only make rider available if they have no other active deliveries
                    $newAvailability = ($activeCount == 0) ? 1 : 0;
                    
                    $updateRiderStmt = $conn->prepare("UPDATE riders SET is_available = ? WHERE rider_id = ?");
                    $updateRiderStmt->bind_param("ii", $newAvailability, $riderId);
                    $updateRiderStmt->execute();
                    
                    error_log("Rider {$riderId} availability updated to {$newAvailability} (active deliveries: {$activeCount})");
                }
            }
        }
