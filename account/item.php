...
$pageTitle = 'Shop';
$currentPage = ''; // Will be set to product name
...
$hasStock = false; // Track if product has any in-stock variants
$totalStock = 0;   // Total available stock
$productCategories = []; // Store category IDs for "You May Also Like"
...
while ($row = $productsResult->fetch_assoc()) {
    if ($product === null) {
        ...
        // Set current page to product name for breadcrumb
        $currentPage = $row['product_name'];
    }
    
    // Handle variants with stock checking
    ...
    if ($row['category_id'] && !isset($categories[$row['category_id']])) {
        ...
        // Store category IDs for "You May Also Like"
...
// Fetch cooking suggestions for this product
$cookingSuggestions = [];
if (!empty($product['product_id'])) {
    $suggestionsQuery = "SELECT * FROM product_cooking_suggestions 
                         WHERE product_id = ? 
                         ORDER BY difficulty_level, dish_name";
    $stmtSuggestions = $conn->prepare($suggestionsQuery);
    $stmtSuggestions->bind_param("i", $product['product_id']);
    $stmtSuggestions->execute();
    $suggestionsResult = $stmtSuggestions->get_result();
    
    while ($suggestion = $suggestionsResult->fetch_assoc()) {
        $cookingSuggestions[] = $suggestion;
    }
    $stmtSuggestions->close();
}

// Fetch "You May Also Like" products (products from same categories, excluding current product)
$relatedProducts = [];
if (!empty($productCategories)) {
    $categoryIds = implode(',', array_unique($productCategories));
    
    $relatedQuery = "SELECT DISTINCT p.*, 
                     ...
                     WHERE pcl.category_id IN ($categoryIds)
                     AND p.product_id != ?
                     AND p.is_deleted = 0
                     GROUP BY p.product_id
                     ORDER BY has_stock DESC, RAND()";
    
    $stmtRelated = $conn->prepare($relatedQuery);
    $stmtRelated->bind_param("i", $product['product_id']);
    $stmtRelated->execute();
    ...
    <div class="grid md:grid-cols-3 gap-4 shadow-lg mb-10">
      <div class="md:col-span-1 p-4 rounded-3xl">
        ...
        <div class="flex flex-col items-center">
            <!-- Main Image with Out of Stock Overlay if needed -->
            <div class="max-w-xl rounded-lg relative">
                ...
                <div class="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 rounded-lg z-10">
                    <span class="out-of-stock-badge">NO STOCK</span>
                </div>
                <?php endif; ?>
                <img id="mainImage"
      ...
      <div class="md:col-span-1 p-4 rounded-3xl">
        ...
        <?php if ($hasStock): ?>
        <!-- Add to Cart Form (only if product has stock) -->
            ...
            <div class="mt-4">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    foreach ($variants as $variant_id => $variant): 
                        ...
                        $isSelected = ($firstInStock && $variant_id === $firstInStock) || (!$firstInStock && $first && $variantHasStock);
        ...
        <?php else: ?>
        <!-- Out of Stock - No Add to Cart form -->
        ...
        <div class="mt-4 pt-4 border-t border-gray-200">
            <div class="flex gap-2">
              <!-- Add to Cart Button - Disabled if no variant selected or variant out of stock -->
              <button type="submit" name="add_to_cart" 
                      class="cursor-pointer w-full py-2 rounded-lg bg-orange-600 hover:bg-orange-700 text-white font-medium transition-all duration-300 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed" title="Add to Cart" disabled>
                      <svg xmlns="http://www.w3.org/2000/svg" class="me-1" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-2"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M6 19m-2 0a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /><path d="M17 19m-2 0a2 2 0 1 0 4 0a2 2 0 1 0 -4 0" /><path d="M17 17h-11v-14h-2" /><path d="M6 5l14 1l-1 7h-13" /></svg>
                      Add to Cart
      ...
      <div class="md:col-span-1 p-4 rounded-3xl">
        ...
        <div class="flex flex-col h-full items-start">
          <h2 class="text-xl font-bold text-gray-800">
            What is <?= htmlspecialchars($product['product_name']) ?>?
          </h2>
          ...
          <div class="mt-5">
              ...
              <div class="flex flex-wrap gap-2">
                  ...
                  if (json_decode($nickname) !== null) {
                      // It's JSON, decode and display as tags
                      $tags = json_decode($nickname, true);
                      if (is_array($tags)) {
                          foreach ($tags as $tag) {
                              echo '<span class="px-3 py-1 bg-orange-50 text-orange-700 text-xs rounded-full border border-orange-200">#' . htmlspecialchars($tag) . '</span>';
                          }
                      } else {
                          echo '<span class="px-3 py-1 bg-gray-100 text-gray-700 text-xs rounded-full">' . htmlspecialchars($nickname) . '</span>';
                      }
                  } else {
                      // It's plain text, display as single tag
                      echo '<span class="px-3 py-1 bg-gray-100 text-gray-700 text-xs rounded-full">' . htmlspecialchars($nickname) . '</span>';
                  }
                  ?>
              </div>
                ...
                <div class="p-6">
                    ...
                    <div class="flex items-center gap-4 mb-4 text-sm text-gray-600">
                        ...
                        <div class="flex items-center">
                            ...
                            Prep: <?= $suggestion['prep_time_minutes'] ?> min
                        </div>
                        <?php endif; ?>
                        ...
                        <div class="flex items-center">
                            ...
                            Cook: <?= $suggestion['cook_time_minutes'] ?> min
                        </div>
                        <?php endif; ?>
                        <?php if (!empty($suggestion['prep_time_minutes']) && !empty($suggestion['cook_time_minutes'])): ?>
                        <div class="flex items-center font-semibold text-orange-600">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linecap="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                            </svg>
                            Total: <?= $suggestion['prep_time_minutes'] + $suggestion['cook_time_minutes'] ?> min
                        </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <p class="text-gray-600 text-sm leading-relaxed">
                            <?= nl2br(htmlspecialchars($suggestion['ingredients'])) ?>
                        </p>
                    </div>
    ...
    <div class="mt-16 mb-16">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            <?php foreach ($relatedProducts as $related): 
                $relatedHasStock = $related['has_stock'] > 0;
                $relatedImage = !empty($related['primary_image']) ? 
                    $baseUrl . "uploads/products/" . $related['primary_image'] : 
                    $baseUrl . "uploads/products/default.png";
                $relatedUrl = $baseUrl . "item/" . strtolower(str_replace(' ', '-', $related['product_name']));
            ?>
            <div class="related-product-card bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100 <?= !$relatedHasStock ? 'opacity-60' : '' ?>">
                <a href="<?= $relatedUrl ?>" class="block">
                    <div class="relative">
                        <img src="<?= htmlspecialchars($relatedImage) ?>" 
                            alt="<?= htmlspecialchars($related['product_name']) ?>"
                            class="w-full h-48 object-cover">
                        <?php if (!$relatedHasStock): ?>
                    ...
                    <div class="p-4">
                        ...
                        <p class="text-orange-600 font-bold">
                            <?= $relatedHasStock ? 'Starting at' : '' ?> ₱<?= number_format($related['min_price'], 2) ?>
                        </p>
                        <?php else: ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.variant-button').forEach(button => {
        button.addEventListener('click', function() {
            ...
            const hasStock = button.dataset.hasStock === 'true';
            ...
            // Enable "Add to Cart" only if variant has stock
            form.querySelector('button[name="add_to_cart"]').disabled = !hasStock;
    ...
    document.querySelector('.decrease-quantity')?.addEventListener('click', function() {
        ...
        const selectedVariant = form.querySelector('.variant-button.selected-variant');
        const stockQuantity = selectedVariant ? parseInt(selectedVariant.dataset.stockQuantity) : 0;
        ...
        const stockError = form.querySelector('.stock-error-message');
        if (newQty > stockQuantity) {
            stockError.textContent = `Only ${stockQuantity} items available in stock`;
    ...
    const firstInStockVariant = Array.from(document.querySelectorAll('.variant-button')).find(btn => btn.dataset.hasStock === 'true');
    if (firstInStockVariant) {
        firstInStockVariant.click();
    } else {
        // If no in-stock variants, disable add to cart button
        const form = document.querySelector('.add-to-cart-form');
        if (form) {
    ...
    document.querySelector('.add-to-cart-form')?.addEventListener('submit', async function(e) {
        ...
        const stockErrorMessage = form.querySelector('.stock-error-message');
        ...
        if (quantity > stockQuantity) {
            stockErrorMessage.textContent = `Only ${stockQuantity} items available in stock`;
