...
<div class="bg-white rounded-xl shadow-sm p-6">
    ...
    <div class="flex flex-wrap gap-3">
        ...
        <?php if ($order['order_status'] === 'OutForDelivery'): ?>
            <!-- Mark as Delivered -->
            <form method="POST" action="./functions/order_process.php" class="inline-block">
                <input type="hidden" name="order_id" value="<?php echo $order['order_id']; ?>">
                <input type="hidden" name="notes" value="Order marked as delivered by admin">
                <button type="submit" name="mark_delivered" 
                    class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        onclick="return confirm('Are you sure this order has been delivered?')">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                    </svg>
                     Mark as Delivered
        ...
        <?php if (in_array($order['order_status'], ['Pending', 'Processing'])): ?>
            ...
            <button type="button" onclick="openCancelModal()" 
                    class="inline-flex items-center px-4 py-2 bg-red-600 text-dark text-sm font-medium rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <?php if ($order['payment_status'] === 'Paid' && $order['payment_method'] !== 'cod'): ?>
            ...
            <button type="button" onclick="openRefundModal()" 
                    class="inline-flex items-center px-4 py-2 bg-yellow-600 text-dark text-sm font-medium rounded-lg hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-yellow-500">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<!-- Cancel Order Modal -->
<div id="cancelModal" class="hidden fixed inset-0 z-100 overflow-y-auto bg-black bg-opacity-50" style="margin: 0">
        ...
        <div class="mt-3">
            ...
            <form method="POST" action="./functions/order_process.php">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="reason" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-red-500 focus:border-red-500" placeholder="Please provide a reason for cancellation..." required></textarea>
                </div>
                
                <div class="flex justify-end space-x-3">
                    <button type="button" onclick="closeCancelModal()" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400">
                        Cancel
                    </button>
                    <button type="submit" name="cancel_order" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700">
                        Cancel Order
                    </button>
                </div>
            </form>
...
<!-- Refund Modal -->
<div id="refundModal" class="hidden fixed inset-0 z-100 overflow-y-auto bg-black bg-opacity-50" style="margin: 0">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            ...
            <form method="POST" action="./functions/order_process.php">
                ...
                <div class="mb-4">
                    ...
                    <select name="refund_amount" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-yellow-500 focus:border-yellow-500" required>
                        <option value="full">Full Refund (₱<?php echo number_format($order['total_price'], 2); ?>)</option>
                    </select>
                </div>
                
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Refund Reason</label>
                    <textarea name="refund_reason" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-yellow-500 focus:border-yellow-500" placeholder="Please provide a reason for the refund..." required></textarea>
                </div>
                
                <div class="flex justify-end space-x-3">
                    <button type="button" onclick="closeRefundModal()" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400">
                        Cancel
                    </button>
                    <button type="submit" name="process_refund" class="px-4 py-2 bg-yellow-600 text-white rounded-md hover:bg-yellow-700">
                        Process Refund
                    </button>
                </div>
            </form>
...
<script>
    ...
    window.onclick = function(event) {
        const modals = ['riderModal', 'cancelModal', 'refundModal'];
