...
// Uses renamed columns: account_first_name, account_last_name, account_email
...
$whereSearch = '';
if ($search !== '') {
    // Uses renamed: account_first_name, account_last_name, account_email
...
<head>
  ...
  <title>Inquiries | St. Joseph Fish Brokerage Inc.</title>
  ...
  <style>
    ...
    .modal-overlay { position:fixed;inset:0;z-index:999; display:flex;align-items:flex-start;justify-content:center; background:rgba(0,0,0,0.55);backdrop-filter:blur(4px);overflow-y:auto;padding:2rem 1rem; }
    .modal-overlay.hidden { display:none; }
    .modal-box { background:white;width:100%;max-width:48rem;border-radius:1.25rem;box-shadow:0 25px 60px rgba(0,0,0,0.2);overflow:hidden; }
    ...
    .modal-close { width:2rem;height:2rem;display:flex;align-items:center;justify-content:center;border-radius:50%;background:#f3f4f6;color:#6b7280;border:none;cursor:pointer;transition:background 0.15s,color 0.15s; }
    .modal-close:hover { background:#fee2e2;color:#dc2626; }
    .modal-body { padding:1.5rem;max-height:75vh;overflow-y:auto; }
    .modal-footer { padding:1rem 1.5rem;border-top:1px solid #f3f4f6;background:#fafafa;display:flex;justify-content:flex-end;gap:0.625rem; }
    .form-label { display:block;font-size:0.8125rem;font-weight:600;color:#374151;margin-bottom:0.375rem; }
    .form-input { width:100%;padding:0.5rem 0.75rem;border:1px solid #e5e7eb;border-radius:0.5rem;font-size:0.875rem;color:#111827;transition:border-color 0.15s,box-shadow 0.15s;outline:none; }
    .form-input:focus { border-color:#ea580c;box-shadow:0 0 0 3px rgba(234,88,12,0.1); }
    .section-title { font-size:0.9375rem;font-weight:700;color:#111827;border-left:3px solid #ea580c;padding-left:0.625rem;margin:1.25rem 0 0.75rem; }
    .btn-primary { padding:0.5rem 1.25rem;background:#ea580c;color:white;border-radius:0.625rem;border:none;font-size:0.875rem;font-weight:600;cursor:pointer;transition:background 0.15s,transform 0.1s; }
    .btn-primary:hover { background:#c2410c; }
    .btn-primary:active { transform:scale(0.97); }
    .btn-secondary { padding:0.5rem 1.25rem;background:white;color:#374151;border-radius:0.625rem;border:1px solid #e5e7eb;font-size:0.875rem;font-weight:500;cursor:pointer;transition:background 0.15s; }
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all">
        <div class="flex items-center justify-between">
          <div><p class="text-sm text-gray-500 font-medium">Total Riders</p><p class="text-2xl font-bold text-gray-900 mt-1"><?= $stats['total'] ?></p></div>
          <div class="w-12 h-12 bg-purple-100 rounded-xl flex items-center justify-center text-xl">🏍️</div>
        </div>
      </div>
      <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all">
        <div class="flex items-center justify-between">
          <div><p class="text-sm text-gray-500 font-medium">Available</p><p class="text-2xl font-bold text-green-600 mt-1"><?= $stats['available'] ?></p></div>
          <div class="w-12 h-12 bg-green-100 rounded-xl flex items-center justify-center text-xl">✅</div>
        </div>
      </div>
      <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all">
        <div class="flex items-center justify-between">
          <div><p class="text-sm text-gray-500 font-medium">Out Delivering</p><p class="text-2xl font-bold text-orange-600 mt-1"><?= $stats['delivering'] ?></p></div>
          <div class="w-12 h-12 bg-orange-100 rounded-xl flex items-center justify-center text-xl">🚚</div>
        </div>
      </div>
      <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all">
        <div class="flex items-center justify-between">
          <div><p class="text-sm text-gray-500 font-medium">Total Delivered</p><p class="text-2xl font-bold text-blue-600 mt-1"><?= $stats['delivered'] ?></p></div>
    ...
    <div class="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
      ...
      <div class="px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-gray-100">
        <div>
          ...
          <p class="text-sm text-gray-500 mt-0.5"><span class="font-semibold text-gray-800"><?= $totalItems ?></span> registered riders</p>
        </div>
        <div class="flex gap-2">
          ...
          <a href="riders.php" class="px-3 py-2 text-sm text-gray-600 hover:text-orange-600 flex items-center gap-1">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> Clear
          </a>
          <?php endif; ?>
          <button onclick="openAddModal()" class="inline-flex items-center gap-2 px-4 py-2 bg-orange-600 text-white text-sm font-semibold rounded-xl hover:bg-orange-700 active:scale-95 transition-all shadow-sm">
        ...
        <table class="min-w-full divide-y divide-gray-100">
          ...
          <tbody class="divide-y divide-gray-100 bg-white">
            ...
            <tr><td colspan="8" class="px-6 py-16 text-center">
              <div class="flex flex-col items-center gap-3">
                <div class="w-14 h-14 bg-purple-50 rounded-2xl flex items-center justify-center">
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#9333ea" stroke-width="1.5"><circle cx="12" cy="10" r="3"/><path d="M5 18v2c0 1.1.9 2 2 2h10a2 2 0 0 0 2-2v-2"/></svg>
                </div>
            ...
            <tr class="rider-row hover:bg-purple-50/40 transition-colors">
              ...
              <!-- Uses renamed: rider_phone (was contact_number), with fallback to account_phone -->
              <td class="px-4 py-4">
                <p class="text-xs text-gray-700"><?= htmlspecialchars($rider['rider_phone'] ?: ($rider['account_phone'] ?? '—')) ?></p>
              </td>
              <td class="px-4 py-4">
                <span class="px-2 py-1 text-xs font-medium bg-indigo-50 text-indigo-700 rounded-full"><?= htmlspecialchars($rider['organization'] ?? '—') ?></span>
              </td>
              ...
              <td class="px-4 py-4 text-center">
                <?php if ($active > 0): ?>
                <span class="flex items-center justify-center gap-1 text-xs text-orange-600"><span class="w-2 h-2 rounded-full bg-orange-500 animate-pulse"></span>Delivering</span>
                <?php elseif ($rider['is_available']): ?>
                <span class="flex items-center justify-center gap-1 text-xs text-green-600"><span class="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span>Available</span>
                <?php else: ?>
                <span class="flex items-center justify-center gap-1 text-xs text-gray-400"><span class="w-2 h-2 rounded-full bg-gray-300"></span>Offline</span>
                <?php endif; ?>
              </td>
              <td class="px-4 py-4 text-right">
                <div class="flex items-center justify-end gap-1.5">
                  <button onclick="openEditModal(<?= $rider['rider_id'] ?>)" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                  ...
                  <button onclick="openDeleteModal(<?= $rider['rider_id'] ?>, '<?= htmlspecialchars(addslashes($rider['display_name'])) ?>', <?= $active ?>)" class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Delete">
      ...
      <div class="px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-t border-gray-100">
        ...
        <div class="flex items-center gap-1.5">
          ...
          if ($page > 1): ?>
          <a href="?page=<?= $page-1 ?><?= $queryPrefix ?>" class="px-3 py-1.5 text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 flex items-center gap-1">
            <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="m15 18-6-6 6-6"/></svg>Prev
          </a>
          <?php endif;
          if ($start > 1) echo '<a href="?page=1'.$queryPrefix.'" class="w-9 h-9 flex items-center justify-center text-sm font-medium bg-white border border-gray-200 rounded-xl hover:bg-gray-50">1</a>';
          if ($start > 2) echo '<span class="text-gray-400 px-1">…</span>';
          for ($i = $start; $i <= $end; $i++): ?>
          <a href="?page=<?= $i ?><?= $queryPrefix ?>" class="w-9 h-9 flex items-center justify-center text-sm font-medium rounded-xl border transition-colors <?= $i == $page ? 'bg-orange-600 text-white border-orange-600' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50' ?>"><?= $i ?></a>
          <?php endfor;
          if ($end < $totalPages - 1) echo '<span class="text-gray-400 px-1">…</span>';
          if ($end < $totalPages) echo '<a href="?page='.$totalPages.$queryPrefix.'" class="w-9 h-9 flex items-center justify-center text-sm font-medium bg-white border border-gray-200 rounded-xl hover:bg-gray-50">'.$totalPages.'</a>';
          if ($page < $totalPages): ?>
          <a href="?page=<?= $page + 1 ?><?= $queryPrefix ?>" class="px-3 py-1.5 text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 flex items-center gap-1">
...
<div id="addRiderModal" class="modal-overlay hidden">
  <div class="modal-box">
    <div class="modal-header">
      <div><h3>Add New Rider</h3><p>Register a new delivery rider</p></div>
      <button class="modal-close" onclick="closeModal('addRiderModal')">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M18 6L6 18M6 6l12 12"/></svg>
      </button>
    </div>
    <div class="modal-body">
      <form id="addRiderForm" action="./functions/add.php" method="POST" enctype="multipart/form-data" class="space-y-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            <!-- Uses renamed POST field: contact_number (maps to rider_phone in add.php) -->
    ...
    <div class="modal-footer">
      ...
      <button type="submit" form="addRiderForm" class="btn-primary">Add Rider</button>
    </div>
  </div>
</div>
...
<div id="editRiderModal" class="modal-overlay hidden">
  <div class="modal-box">
    <div class="modal-header">
      <div><h3>Edit Rider</h3><p>Update rider information</p></div>
    ...
    <div id="editRiderContent" class="modal-body">
      <div class="flex items-center justify-center py-12 text-gray-400">
        <svg class="animate-spin mr-3" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
        Loading rider data...
      </div>
    </div>
  </div>
...
<div id="deleteRiderModal" class="modal-overlay hidden">
  <div class="modal-box" style="max-width:28rem">
    <div class="modal-header">
      <div><h3>Remove Rider</h3><p>This action cannot be undone</p></div>
    ...
    <div class="modal-body text-center">
      <div class="w-14 h-14 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-4">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
      </div>
...
// ── openEditModal — fetch_riders.php returns rider_name, rider_phone aliases ──
function openEditModal(riderId) {
  ...
  fetch(`./functions/fetch_riders.php?rider_id=${riderId}`)
    ...
    .then(data => {
      ...
      // fetch_riders.php returns legacy aliases (full_name, contact_number) for backward compat
      // but we use the renamed field names in the form → update.php maps them to rider_name/rider_phone
      content.innerHTML = `
        <form action="./functions/update.php" method="POST" enctype="multipart/form-data" class="space-y-4">
          ...
          <div>
            ...
            <!-- POST name 'full_name' is mapped to rider_name in update.php -->
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <!-- POST name 'contact_number' is mapped to rider_phone in update.php -->
